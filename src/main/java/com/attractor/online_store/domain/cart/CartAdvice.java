package com.attractor.online_store.domain.cart;
...
public class CartAdvice {
    ...
    public List<Product> getCartModel(HttpSession session) {
        var list = session.getAttribute(Constants.CART_ID);
        if (list == null) {
            session.setAttribute(Constants.CART_ID, new ArrayList<>());
        }
        return (List<Product>) session.getAttribute(Constants.CART_ID);
    }
